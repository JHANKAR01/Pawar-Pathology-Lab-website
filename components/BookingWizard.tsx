...
import { Check, ArrowRight, ArrowLeft, Calendar, CreditCard, User, ClipboardList, Wallet, CheckCircle } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300 max-h-[90vh] flex flex-col">
        {/* Progress Bar */}
        <div className="bg-gray-50 px-6 md:px-8 py-6 border-b border-gray-100 shrink-0">
              ...
              <React.Fragment key={i}>
                <div className="flex flex-col items-center gap-1 min-w-[4rem]">
                  <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center text-sm font-bold transition-all ${
                    ...
                    {step > i + 1 ? <Check className="w-4 h-4 md:w-5 md:h-5" /> : i + 1}
                  </div>
                  <span className={`text-[9px] md:text-[10px] font-bold uppercase whitespace-nowrap ${step === i + 1 ? 'text-red-600' : 'text-gray-400'}`}>{s.title}</span>
                </div>
                {i < steps.length - 1 && <div className={`flex-1 h-0.5 mb-4 min-w-[1rem] ${step > i + 1 ? 'bg-green-500' : 'bg-gray-200'}`} />}
              </React.Fragment>
            ))}
          </div>
        </div>
          ...
          {step === 4 && (
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-gray-500 text-sm">Choose how you would like to pay for your tests.</p>
              </div>
              
              <div className="grid grid-cols-1 gap-4">
                <button
                  onClick={() => setPaymentMethod('online')}
                  className={`p-4 rounded-2xl border-2 flex items-center justify-between transition-all ${paymentMethod === 'online' ? 'border-red-600 bg-red-50 ring-1 ring-red-600' : 'border-gray-100 hover:border-gray-200'}`}
                >
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm text-red-600 border border-gray-100">
                      <CreditCard className="w-6 h-6" />
                    </div>
                    <div className="text-left">
                      <p className="font-bold text-gray-900">Pay Online</p>
                      <p className="text-xs text-gray-500">Secure Razorpay Gateway</p>
                    </div>
                  </div>
                  {paymentMethod === 'online' && <CheckCircle className="text-red-600 w-6 h-6" />}
                </button>

                <button
                  onClick={() => setPaymentMethod('cash')}
                  className={`p-4 rounded-2xl border-2 flex items-center justify-between transition-all ${paymentMethod === 'cash' ? 'border-red-600 bg-red-50 ring-1 ring-red-600' : 'border-gray-100 hover:border-gray-200'}`}
                >
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm text-green-600 border border-gray-100">
                      <Wallet className="w-6 h-6" />
                    </div>
                    <div className="text-left">
                      <p className="font-bold text-gray-900">Pay on Collection</p>
                      <p className="text-xs text-gray-500">Cash / UPI during sample pickup</p>
                    </div>
                  </div>
                  {paymentMethod === 'cash' && <CheckCircle className="text-red-600 w-6 h-6" />}
                </button>
              </div>

              <div className="bg-gray-50 p-6 rounded-2xl border border-dashed border-gray-300 text-center mt-4">
        ...
        <div className="bg-gray-50 px-6 md:px-8 py-4 border-t border-gray-100 flex justify-between shrink-0">
          <button 
            disabled={step === 1}
            onClick={prevStep}
            className={`flex items-center gap-2 font-bold px-4 py-2 rounded-xl transition-all ${step === 1 ? 'opacity-0 pointer-events-none' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-200'}`}
          >
          ...
          {step < 4 ? (
            <button 
              onClick={nextStep}
              className="bg-red-600 text-white px-6 md:px-8 py-3 rounded-xl font-bold flex items-center gap-2 hover:bg-red-700 shadow-lg"
            >
            ...
            <button 
              ...
              className={`text-white px-6 md:px-8 py-3 rounded-xl font-bold flex items-center gap-2 shadow-lg transition-all ${paymentMethod === 'online' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'}`}
            >
              {paymentMethod === 'online' ? 'Pay & Confirm' : 'Confirm Booking'} <Check className="w-5 h-5" />
            </button>
          )}
        </div>
      </div>
