...
import {
  ...
  LayoutDashboard, HeartHandshake, Settings as SettingsIcon, Clock, CheckCircle2, Download, BarChart3
} from 'lucide-react';
...
export default function AdminPage() {
  ...
  // Phase 9: Analytics State
  const [analyticsData, setAnalyticsData] = useState<{ summary: any, trends: any[] } | null>(null);
  ...
  const handleExport = async () => {
    try {
      const res = await fetch(`/api/admin/export?startDate=${dateRange.startDate}&endDate=${dateRange.endDate}`);
      ...
      a.href = url;
      a.download = `bookings_report_${dateRange.startDate}_to_${dateRange.endDate}.xlsx`;
        ...
        <div className="bg-white rounded-3xl p-8 shadow-large border border-slate-200 space-y-8">
          ...
          <AnimatePresence mode="wait">
            ...
            className="space-y-8"
              >
            {/* Filters & Actions */}
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-50 p-6 rounded-3xl border border-slate-200">
              <div className="flex items-center gap-4 w-full md:w-auto">
                <div className="relative flex-1 md:flex-none">
                  ...
                  <input
                    ...
                    className="pl-10 pr-4 py-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-700 outline-none focus:border-clinical-rose text-sm w-full"
                  />
                </div>
              ...
              <button
                ...
                className="w-full md:w-auto flex items-center justify-center gap-2 bg-green-600 text-white px-6 py-3 rounded-xl font-bold uppercase tracking-wider text-xs hover:bg-green-700 transition-all shadow-lg"
              >
            ...
            {/* Stats Grid */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <motion.div className="card-premium p-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-3">Total Revenue</p>
                <p className="text-4xl font-black text-slate-900">₹{analyticsData?.summary?.totalRevenue?.toLocaleString() || 0}</p>
              </motion.div>
              <motion.div className="card-premium p-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                <p className="text-warning text-[10px] font-black uppercase tracking-widest mb-3">Pending Approval</p>
                <p className="text-4xl font-black text-slate-900">{analyticsData?.summary?.pendingCount || 0}</p>
              </motion.div>
              <motion.div className="card-premium p-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
                <p className="text-success text-[10px] font-black uppercase tracking-widest mb-3">Completed Cycles</p>
                <p className="text-4xl font-black text-slate-900">{analyticsData?.summary?.completedCount || 0}</p>
              </motion.div>
              <motion.div className="card-premium p-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
                <p className="text-purple-500 text-[10px] font-black uppercase tracking-widest mb-3">Active Patients</p>
                <p className="text-4xl font-black text-slate-900">{analyticsData?.summary?.totalPatients || 0}</p>
              </motion.div>
            </div>
            ...
            <div className="card-premium p-8">
              <h3 className="text-lg font-black text-slate-900 mb-6 flex items-center gap-2">
                <BarChart3 className="text-clinical-rose" /> Daily Performance
              </h3>
              <div className="h-64 flex items-end gap-2 overflow-x-auto pb-4">
                {analyticsData?.trends?.map((day: any) => (
                  <div key={day.date} className="flex flex-col items-center group relative min-w-[40px] flex-1">
                    {/* Tooltip */}
                    <div className="absolute bottom-full mb-2 bg-slate-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10 pointer-events-none">
                      {day.date}: ₹{day.revenue} ({day.count})
                    </div>
                    <div
                      className="w-full bg-clinical-rose/20 hover:bg-clinical-rose transition-colors rounded-t-lg relative group-hover:shadow-lg"
                      style={{ height: `${Math.max(10, (day.revenue / (Math.max(...analyticsData.trends.map((t: any) => t.revenue)) || 1)) * 100)}%` }}
                    ></div>
                    <span className="text-[10px] text-slate-400 font-bold mt-2 rotate-45 origin-left truncate w-full block">{new Date(day.date).getDate()}/{new Date(day.date).getMonth() + 1}</span>
                  </div>
                ))}
                {(!analyticsData?.trends || analyticsData.trends.length === 0) && (
                  <div className="w-full h-full flex items-center justify-center text-slate-400 italic">No trend data available</div>
                )}
              </div>
            </div>
          </motion.div>
          ...
          {activeTab === 'Bookings' && (
            ...
            >
              ...
              <PaginationControls
                ...
                totalPages={Math.ceil(totalBookings / bookingsPerPage)}
                onPageChange={setCurrentBookingPage}
                limit={bookingsPerPage}
                onLimitChange={setBookingsPerPage}
            ...
            >
              ...
              >
                ...
                <PaginationControls
                  ...
                  totalPages={Math.ceil(totalPartners / partnersPerPage)}
                  onPageChange={setCurrentPartnerPage}
                  limit={partnersPerPage}
                  onLimitChange={setPartnersPerPage}
                  ...
                  <div className="space-y-8">
                    ...
                    {isMaster ? (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="col-span-full md:col-span-2 bg-slate-50 p-6 rounded-2xl border border-slate-200">
                          <div className="flex items-center gap-3 mb-6">
                            <ShieldCheck className="text-slate-400" />
                            <div>
                              <h4 className="font-bold text-slate-900">Security & Rate Limiting</h4>
                              <p className="text-xs text-slate-500">Configure API request quotas to prevent abuse.</p>
                            </div>
                          </div>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                              <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">IP Requests / Minute</label>
                              <input
                                type="number"
                                className="w-full bg-white border-2 border-slate-200 rounded-xl px-4 py-3 font-bold text-slate-900 outline-none focus:border-clinical-rose"
                                value={(config as any).rateLimit?.ipRequestsPerMinute ?? 60}
                                onChange={(e) => updateConfig('rateLimit', { ipRequestsPerMinute: Number(e.target.value) })}
                              />
                            </div>
                            <div>
                              <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Global Requests / Minute</label>
                              <input
                                type="number"
                                className="w-full bg-white border-2 border-slate-200 rounded-xl px-4 py-3 font-bold text-slate-900 outline-none focus:border-clinical-rose"
                                value={(config as any).rateLimit?.globalRequestsPerMinute ?? 1000}
                                onChange={(e) => updateConfig('rateLimit', { globalRequestsPerMinute: Number(e.target.value) })}
                              />
                            </div>
                          </div>
                        </div>
